...
public final class ReciprocalArraySum {
  ...
  protected static double parManyTaskArraySum(final double[] input, final int numTasks) {
    List<ReciprocalArraySumTask> tasks = createTasks(input, numTasks);
  ...
  private static List<ReciprocalArraySumTask> createTasks(final double[] input,
      final int numTasks) {
    List<ReciprocalArraySumTask> tasks = new ArrayList<>();
    int startIndex;
    int endIndex;
    for (int chunk = 0; chunk < numTasks; chunk++) {
      startIndex = getChunkStartInclusive(chunk, numTasks, input.length);
      endIndex = getChunkEndExclusive(chunk, numTasks, input.length);
